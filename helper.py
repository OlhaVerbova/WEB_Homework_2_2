...
from output_info import Show_to_console #Homework 1 
...
class AddressBook(UserDict):
    ...
    def delete_contact(self, name):
        ...
        if name in self.data:
            ...
            return f"Contact {name} deleted from address_bot - saved."
        else:            
    ...
    def set_value(self, value):
        try:
            ...
            try:
                ...
                return f"Invalid date format. Please use 'dd.mm.yy'"
...
def handle_command(address_book, command):
   # Розділити введену команду на частини
    ...
    if not parts:
        return f"Enter a command. Type 'helper' for a list of available commands."            
...
# 15.10.23 Yulia
        ...
        return address_book.delete_contact(name)
...
#14.10.23 Nazar
        ...
        return "Unknown command. Type 'helper' for a list of available commands."
...
def main():    
    ...
    show_to_user = Show_to_console() #Homework_1
   
    command_prompt = "Enter a command: "
    
    show_to_user.greating_message() #Homework_1
    show_to_user.saved_infornation_addressBook(filename, address_book) #Homework_1 
        
    while True:
        user_input = input(command_prompt).strip()
        if not user_input:
            if not user_input:
                show_to_user.show_error_command() #Homework_1             
            continue  
        response = handle_command(address_book, user_input)        
        show_to_user.show_all_information(response) #Homework_1  
